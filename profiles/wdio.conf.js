exports.config = {
  ...
  afterScenario: function(uri, feature, scenario, result, sourceLocation) {
    ...
    if (result.status != "passing") {
      var fs = require(process.cwd() + "/automation/utils/report");
      var dir = "./reports/json/screens/";
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
      }
      var screenshootName = `${scenario.name}-${fs.screenshootName(browser.capabilities)}`
  ...
  onComplete: async function(exitCode, config, capabilities, results) {
    if (process.env.UPDATE_TESTRAIL == 1) {
